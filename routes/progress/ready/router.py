...
async def delete_ready_progress(user_id: str, progress_name: str) -> Answer:
    """Удаления отметки (даты) выполнения прогресса"""
    if not is_user_id_in_vault(user_id=user_id):
        return Answer(status_code=400, info=f"the user, {user_id}, not found")
    if not is_progress_in_user_id(user_id=user_id, progress_name=progress_name):
        return Answer(
    ...
    try:
        today = delete_ready_user_progress(user_id=user_id, progress_name=progress_name)
        return Answer(
            status_code=200,
            info=f"successful delete today date, {today}, in progress, {progress_name}, for user, {user_id}",
        )
    except ValueError as valueError:
        return Answer(status_code=400, info=str(valueError))
